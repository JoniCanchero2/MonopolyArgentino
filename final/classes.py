...
class Player:
    ...
    def pay(self, amount):
        #paga
        self.dinero -= amount
        return self.dinero <= 0 # Retorna si el jugador queda en bancarrota
...
class Game:
    ...
    def __init__(self, board_data, players_data, pantalla, fonts):
        ...
        self.trade_state = False
        self.selected_my_property = None
        self.selected_their_property = None
    ...
    def roll_dice(self):
        ...
        # 1) Lógica para salida de la Carcel
    ...
    def next_turn(self):
        ...
        # 3)Pasa al siguiente índice activo
        while True:
    ...
    def get_player_properties(self, player_id):
        ...
        propiedades = []
        for i, casilla in enumerate(self.board):
            if casilla["propietario"] == player_id and casilla["tipo"] in ["Propiedad", "Ferrocarril", "Servicio"]:
                propiedades.append({
                    "index": i,
                    "nombre": casilla["nombre"],
                    "precio": casilla["precio"],
                    "tipo": casilla["tipo"]
                })
        return propiedades
    ...
    def start_trade(self):
        player = self.current_player
        propiedades_propias = self.get_player_properties(player.id)
        
        if not propiedades_propias:
            self.current_message = "No tienes propiedades para intercambiar. Presiona 'S' para continuar."
            self.trade_state = False
            self.interaction_done = True  # Si no hay propiedades
            return
        
        self.current_message = "INTERCAMBIO: Selecciona TU propiedad con 1-9"
        self.selected_my_property = None
        self.selected_their_property = None
        self.selected_target_player = None

    def select_my_property_for_trade(self, property_index):
        player = self.current_player
        propiedades = self.get_player_properties(player.id)
        
        if 0 <= property_index < len(propiedades):
            self.selected_my_property = propiedades[property_index]["index"]
            prop_nombre = self.board[self.selected_my_property]["nombre"]
            self.current_message = f"Tu propiedad: {prop_nombre}. Ahora elige jugador (F1-F4)"
        else:
            self.current_message = "Número de propiedad inválido."

    def select_trade_partner(self, player_number):
        target_players = [p for p in self.players if p.activo and p.id != self.current_player.id]
        
        if 0 <= player_number < len(target_players):
            self.selected_target_player = target_players[player_number]
            # Obtener propiedades del otro jugador
            their_properties = self.get_player_properties(self.selected_target_player.id)
            
            if not their_properties:
                self.current_message = f"{self.selected_target_player.nombre} no tiene propiedades. Intercambio cancelado."
                self.cancel_trade()
                return
            
            self.current_message = f"Intercambiando con {self.selected_target_player.nombre}. Selecciona SU propiedad con 1-9"
        else:
            self.current_message = "Jugador objetivo inválido."

    def select_their_property_for_trade(self, property_index):
        
        their_properties = self.get_player_properties(self.selected_target_player.id)
        
        if 0 <= property_index < len(their_properties):
            self.selected_their_property = their_properties[property_index]["index"]
            prop_nombre = self.board[self.selected_their_property]["nombre"]
            my_prop_nombre = self.board[self.selected_my_property]["nombre"]
            
            self.current_message = f"Intercambio: {my_prop_nombre} ↔ {prop_nombre}. ENTER para confirmar, ESC para cancelar"
        else:
            self.current_message = "Número de propiedad inválido."

    def complete_trade(self):
        if (self.selected_my_property is None or 
            self.selected_their_property is None or
            self.selected_target_player is None):
            return
        
        # Obtener nombres para el intercambio
        my_prop_nombre = self.board[self.selected_my_property]["nombre"]
        their_prop_nombre = self.board[self.selected_their_property]["nombre"]
        
        # Hace el intercambio
        temp_owner = self.board[self.selected_my_property]["propietario"]
        self.board[self.selected_my_property]["propietario"] = self.board[self.selected_their_property]["propietario"]
        self.board[self.selected_their_property]["propietario"] = temp_owner
        
        self.current_message = f" Intercambio exitoso {my_prop_nombre} ↔ {their_prop_nombre}"
        self.cancel_trade()
    ...
    def process_suerte_card(self):
        ...
        # 4) Interacción de dinero para todos
        ...
        # 7) Acciones de Movimiento
            ...
            if tipo_movimiento == "absolute_jail":
                ...
                # Carta 8: Ir a Olivos (Posición 39) pudiendo cobrar
                ...
                # Carta 9: Retrocede 3 casilleros
                roll = value # -3
    ...
    def handle_interaction(self):
        ...
        if casilla["tipo"] in ["Propiedad", "Ferrocarril", "Servicio"]:
            ...
            # 2) Tiene dueño: Pagar alquiler y transferir dinero
            ...
            if casilla["nombre"] == "SUERTE":
             ...
             self.current_message += f" ¡Caíste en {casilla['nombre']}! Presiona 'N' para intercambiar propiedades o 'S' para continuar."
    ...
    def draw_ui(self):
        ...
        # Lógica para dividir el mensaje si es muy largo
        ...
        # Crea una superficie semitransparente para el fondo del mensaje
        if line2_text:
        ...
        # Instrucciones
        # Nuevo Interfaz de intercambio
        if self.trade_state:
            # Fondo para mensaje de intercambio
            message = self.current_message
            text_msg = self.SMALL_FONT.render(message, True, BLACK)
            
            ancho_fondo = text_msg.get_width() + 40
            alto_fondo = 150  # Más alto para las instrucciones
            
            fondo_mensaje = pygame.Surface((ancho_fondo, alto_fondo), pygame.SRCALPHA)
            fondo_mensaje.fill((200, 255, 200, 230))  # Verde claro semitransparente
            
            x_fondo = self.ANCHO/2 - ancho_fondo/2
            y_fondo = self.ALTURA/2 + 10
            self.pantalla.blit(fondo_mensaje, (x_fondo, y_fondo))
            pygame.draw.rect(self.pantalla, (0, 150, 0), (x_fondo, y_fondo, ancho_fondo, alto_fondo), 2)
            
            # Dibuja mensaje principal
            self.pantalla.blit(text_msg, (self.ANCHO/2 - text_msg.get_width()/2, self.ALTURA/2 + 20))
            
            # Muestra propiedades según la etapa del intercambio
            if self.selected_my_property is None:
                # Muestra propiedades propias
                player = self.current_player
                propiedades = self.get_player_properties(player.id)
                
                instrucciones = "TUS PROPIEDADES:"
                text_inst = self.TINY_FONT.render(instrucciones, True, BLUE)
                self.pantalla.blit(text_inst, (self.ANCHO/2 - text_inst.get_width()/2, self.ALTURA/2 + 40))
                
                for i, prop in enumerate(propiedades[:9]):
                    prop_text = f"{i+1}. {prop['nombre']}"
                    text_prop = self.TINY_FONT.render(prop_text, True, BLACK)
                    self.pantalla.blit(text_prop, (self.ANCHO/2 - text_prop.get_width()/2, self.ALTURA/2 + 55 + i*12))
                
                instrucciones = "Selecciona tu propiedad con 1-9, ESC para cancelar"
                text_inst = self.TINY_FONT.render(instrucciones, True, (200, 0, 0))
                self.pantalla.blit(text_inst, (self.ANCHO/2 - text_inst.get_width()/2, self.ALTURA/2 + 130))
                
            elif self.selected_target_player is None:
                # Muestra jugadores disponibles
                instrucciones = "SELECCIONA JUGADOR:"
                text_inst = self.TINY_FONT.render(instrucciones, True, BLUE)
                self.pantalla.blit(text_inst, (self.ANCHO/2 - text_inst.get_width()/2, self.ALTURA/2 + 40))
                
                target_players = [p for p in self.players if p.activo and p.id != self.current_player.id]
                
                for i, target in enumerate(target_players[:4]):
                    player_text = f"F{i+1}. {target.nombre}"
                    text_player = self.TINY_FONT.render(player_text, True, BLACK)
                    self.pantalla.blit(text_player, (self.ANCHO/2 - text_player.get_width()/2, self.ALTURA/2 + 55 + i*12))
                
                instrucciones = "Selecciona jugador con F1-F4, ESC para cancelar"
                text_inst = self.TINY_FONT.render(instrucciones, True, (200, 0, 0))
                self.pantalla.blit(text_inst, (self.ANCHO/2 - text_inst.get_width()/2, self.ALTURA/2 + 130))
                
            elif self.selected_their_property is None:
                # Muestra propiedades del otro jugador
                their_properties = self.get_player_properties(self.selected_target_player.id)
                
                instrucciones = f"PROPIEDADES DE {self.selected_target_player.nombre.upper()}:"
                text_inst = self.TINY_FONT.render(instrucciones, True, BLUE)
                self.pantalla.blit(text_inst, (self.ANCHO/2 - text_inst.get_width()/2, self.ALTURA/2 + 40))
                
                for i, prop in enumerate(their_properties[:9]):
                    prop_text = f"{i+1}. {prop['nombre']}"
                    text_prop = self.TINY_FONT.render(prop_text, True, BLACK)
                    self.pantalla.blit(text_prop, (self.ANCHO/2 - text_prop.get_width()/2, self.ALTURA/2 + 55 + i*12))
                
                instrucciones = "Selecciona propiedad con 1-9, ESC para cancelar"
                text_inst = self.TINY_FONT.render(instrucciones, True, (200, 0, 0))
                self.pantalla.blit(text_inst, (self.ANCHO/2 - text_inst.get_width()/2, self.ALTURA/2 + 130))
                
            else:
                # Muestra resumen del intercambio
                my_prop_nombre = self.board[self.selected_my_property]["nombre"]
                their_prop_nombre = self.board[self.selected_their_property]["nombre"]
                
                trade_text = f"INTERCAMBIO PROPUESTO:"
                my_prop_text = f"TÚ das: {my_prop_nombre}"
                their_prop_text = f"RECIBES: {their_prop_nombre}"
                
                text_trade = self.SMALL_FONT.render(trade_text, True, BLUE)
                text_my_prop = self.SMALL_FONT.render(my_prop_text, True, BLACK)
                text_their_prop = self.SMALL_FONT.render(their_prop_text, True, (0, 100, 0))
                
                self.pantalla.blit(text_trade, (self.ANCHO/2 - text_trade.get_width()/2, self.ALTURA/2 + 40))
                self.pantalla.blit(text_my_prop, (self.ANCHO/2 - text_my_prop.get_width()/2, self.ALTURA/2 + 60))
                self.pantalla.blit(text_their_prop, (self.ANCHO/2 - text_their_prop.get_width()/2, self.ALTURA/2 + 80))
                
                instrucciones = "ENTER: Confirmar intercambio, ESC: Cancelar"
                text_inst = self.TINY_FONT.render(instrucciones, True, (200, 0, 0))
                self.pantalla.blit(text_inst, (self.ANCHO/2 - text_inst.get_width()/2, self.ALTURA/2 + 130))
        
        # Instrucciones
        elif not self.game_over and not self.purchase_state and not self.interaction_done:
